  ...
  function on_runtime (data, type, paths) {
    for (let i = 0; i < paths.length; i++) {
      ...
      if (path.endsWith('vertical_scroll_value.json')) vertical_scroll_value = value 
        ...
        shadow.querySelectorAll('.node.selected').forEach(n => n.classList.remove('selected'))
        selected_instance_paths.forEach(p => {
          const node = shadow.querySelector(`[data-instance_path="${CSS.escape(p)}"]`)
