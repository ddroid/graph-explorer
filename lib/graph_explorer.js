...
******************************************************************************/
  ...
  function create_node ({
    ...
    // For search mode interactions (hubs/subs toggle), use search_entry_states
    const interaction_states = mode === 'search' ? search_entry_states : instance_states
    const interaction_state = get_or_create_state(interaction_states, instance_path)
    const el = document.createElement('div')
  ...
  function build_search_view_recursive ({
    ...
    const hub_results = should_expand_hubs ? (entry.hubs || []).flatMap((hub_path, i, arr) =>
      build_search_view_recursive({
        ...
        all_entries,
        original_view,
        is_expanded_child: true // Mark as expanded child
      })
    ) : []
    ...
    if (should_expand_subs) {
      ...
      sub_results = (entry.subs || []).map((sub_path, i, arr) => {
        const sub_result = build_search_view_recursive({
          query,
          base_path: sub_path,
          parent_instance_path: instance_path,
          depth: depth + 1,
      ...
      sub_results = (entry.subs || []).flatMap((sub_path, i, arr) =>
        build_search_view_recursive({
          ...
          all_entries,
          original_view,
          is_expanded_child: false // Regular search
        })
      )
    }
