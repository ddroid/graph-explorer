...
******************************************************************************/
  ...
  function on_mode ({ data, paths }) {
    ...
    for (let i = 0; i < paths.length; i++) {
        const path = paths[i]
        const value = data[i]
        if (path.endsWith('current_mode.json')) {
            new_current_mode = value
        } else if (path.endsWith('previous_mode.json')) {
            new_previous_mode = value
        }
    }

    if (new_previous_mode) {
        previous_mode = new_previous_mode
    }
