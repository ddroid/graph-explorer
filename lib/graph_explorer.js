...
******************************************************************************/
  ...
  function on_mode ({ data, paths }) {
    ...
    if (new_current_mode) {
      const valid_modes = ['default', 'menubar', 'search']
      if (!valid_modes.includes(new_current_mode)) {
  ...
  function get_or_create_state (states, instance_path) {
    if (!states[instance_path]) {
      console.warn(
        `State not found for instance: ${instance_path}. Creating default state.`
      )
      states[instance_path] = {
        expanded_subs: false,
        expanded_hubs: false
      }
    }
...
******************************************************************************/
  function render_menubar () {
    ...
    if (mode !== 'search') {
      menubar.replaceChildren(search_button)
      return
    }
  ...
  function toggle_search_mode () {
    ...
    update_mode_state('current_mode', mode === 'search' ? previous_mode : 'search')
    search_state_instances = instance_states
  }
  ...
  function on_search_input (event) {
    ...
    drive_updated_by_search = true
    update_mode_state('search_query', search_query)
  ...
  ******************************************************************************/
  function select_node (ev, instance_path) {
    ...
    if (ev.ctrlKey) {
      ...
      update_runtime_state('selected_instance_paths', [...new_selected])
    } else {
  ...
  function handle_confirm (ev, instance_path) {
    ...
    update_runtime_state('selected_instance_paths', [...new_selected])
    update_runtime_state('confirmed_selected', [...new_confirmed])
  }
