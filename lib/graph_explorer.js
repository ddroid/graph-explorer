...
async function graph_explorer(opts) {
  ...
  function reset() {
    vertical_scroll_value = 0
    horizontal_scroll_value = 0
    view = []
    Object.keys(instance_states).forEach(k => delete instance_states[k])
    const root_path = '/'
    if (all_entries[root_path]) {
      instance_states[root_path] = { expanded_subs: true, expanded_hubs: false }
      build_and_render_view()
  ...
  function select_node(instance_path, base_path) {
    ...
    selected_instance_path = instance_path
    ...
    const new_node = shadow.querySelector(`[data-instance_path="${CSS.escape(instance_path)}"]`)
